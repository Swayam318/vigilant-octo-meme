...
```
### 4. Train the Visual Feature Extractor:
...
```
### 5. Train the Hybrid Ensemble (Final Model):
   Run hybrid_training.ipynb.
```
Action: 1. Baseline Model: Trains a Random Forest on tabular data (R2 Score of approx 0.86).
        2. Feature Extraction: Loads the trained CNN and extracts a 64-dimensional visual embedding vector for every house.
        3. Clustering: Performs Geospatial K-Means Clustering to create "Neighborhood" features.
        4. Ensemble Training: Trains a Voting Regressor (XGBoost + Random Forest + HistGradientBoosting) on the combined data.
Output: Prints the Ensemble results and generates the final submission file `22322030_final.csv`.
```
### 6. Explainability:
   Run explainability.ipynb.
```
...
Passed to a Voting Regressor (w1 * XGB + w2 * HGB + w3 * RF$) for final price prediction.
```
